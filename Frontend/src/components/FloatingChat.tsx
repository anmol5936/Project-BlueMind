...
const FloatingChat: React.FC = () => {
  ...
  }); // Adjusted initial position for closed state
  ...
  const [chatHistory, setChatHistory] = useState<
    { type: "user" | "bot"; message: string; image?: string }[]
  >([
    {
      type: "bot",
      message:
    ...
    if (isDragging && chatRef.current) {
      ...
      animationFrameRef.current = requestAnimationFrame(() => {
        const newX = Math.max(
          ...
          Math.min(
            ...
            window.innerWidth - (isOpen ? size.width : 64)
          )
        ...
        const newY = Math.max(
          ...
          Math.min(
            ...
            window.innerHeight - (isOpen ? size.height : 64)
          )
        );
      ...
      animationFrameRef.current = requestAnimationFrame(() => {
        const newWidth = Math.max(
          300,
          size.width + (e.clientX - resizeStart.x)
        );
        const newHeight = Math.max(
          400,
          size.height + (e.clientY - resizeStart.y)
        );
  ...
  const handleAnalyze = async (voiceQuery?: string) => {
    ...
    const finalQuery =
      voiceQuery ||
      query ||
      (language === "hi" ? "इसे समझाएं।" : "Explain this.");
    const userMessage = {
      type: "user" as const,
      message: finalQuery,
      image: capturedImage,
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1">
          {paragraphs.map((paragraph, pIndex) => {
            ...
            if (
              cleanLines.length > 1 &&
              cleanLines.some((line) => line.includes(":"))
            ) {
              return (
        ...
        >
          ...
          >
            <h3 className="text-lg font-semibold">
              {language === "hi"
                ? "फार्म हेल्पर विजुअल असिस्टेंट"
                : "Farm Helper Visual Assistant"}
            </h3>
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                title={
                  language === "hi"
                    ? captureMode === "webcam"
                      ? "विंडो कैप्चर पर स्विच करें"
                      : "कैमरा पर स्विच करें"
                    : captureMode === "webcam"
                    ? "Switch to window capture"
                    : "Switch to camera"
                }
              >
